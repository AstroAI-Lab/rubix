...
from .helpers import N_PARAMS_AXIS, N_WAVE_AXIS, _smoothstep
...
class Cardelli89(BaseExtRvModel):
    r"""
    Calculate the R(V) dependent CCM89 extinction curve of the Milky Way.

    Args:
        Rv (float): Total-to-selective extinction ``A(V)/E(B-V)``.

    Attributes:
        wave_range_l (float): Minimum supported wavelength in micron.
        wave_range_h (float): Maximum supported wavelength in micron.
        Rv (float): Configured total-to-selective extinction.
        Rv_range_l (float): Minimum valid Rv.
        Rv_range_h (float): Maximum valid Rv.

    Notes:
        Based on Cardelli, Clayton, and Mathis (1989, ApJ, 345, 245).

    Returns:
        Float[Array, "n_wave"]: A(x)/A(V) extinction curve [mag]
    ...
    # wave_range: Float[Array, "2"] = equinox.field(
    ...
    def evaluate(self, wave: Float[Array, N_WAVE_AXIS]) -> Float[Array, N_WAVE_AXIS]:
        """
        Evaluate the CCM89 R(V)-dependent extinction curve.

        Args:
            wave (Float[Array, N_WAVE_AXIS]): Wavelengths in microns.

        Returns:
            Float[Array, N_WAVE_AXIS]: Extinction values A(lambda)/A(V).
        """
        ...
        a = jnp.where(
            fuv_mask,
            -1.073 - 0.628 * y + 0.137 * y**2 - 0.070 * y**3,
            a,
        )
        b = jnp.where(
            fuv_mask,
            13.670 + 4.257 * y - 0.420 * y**2 + 0.374 * y**3,
            b,
        )
...
class Gordon23(BaseExtRvModel):
    r"""
    R(V) dependent Milky Way extinction curve from Gordon et al. (2023).
    ...
    Notes:
        This implementation follows Gordon et al. (2023, ApJ, 950, 86).
    ...
    Example:
        ...
        .. plot::
            ...
            # generate the curves and plot them
            lam = (
                np.logspace(np.log10(0.0912), np.log10(30.0), num=1000)
                * u.micron
            )
    ...
    # wave_range: ClassVar[Float[Array, "2"]] = equinox.field(
    #     converter=jnp.asarray,
    #     static=True,
    #     default=jnp.array(wave_range_G23),
    # )
    # Rv_range: ClassVar[Float[Array, "2"]] = equinox.field(
    #     converter=jnp.asarray,
    #     static=True,
    ...
    def evaluate(self, wave: Float[Array, N_WAVE_AXIS]) -> Float[Array, N_WAVE_AXIS]:
        """
        Evaluate the Gordon et al. (2023) extinction curve.

        Args:
            wave (Float[Array, N_WAVE_AXIS]): Wavelengths in microns.

        Returns:
            Float[Array, "n_wave"]: Extinction values A(lambda)/A(V).
        """
        # setup the a & b coefficient vectors
        a = jnp.zeros(wave.shape)
        b = jnp.zeros(wave.shape)
        ...
        def compound_polynomial_drude_model(
            ...
            params: Float[Array, N_PARAMS_AXIS],
        ) -> Float[Array, N_WAVE_AXIS]:
            """
            Combine a polynomial and three Drude profiles into one model.

            Args:
                x (Float[Array, N_WAVE_AXIS]): Input wavelengths in
                    wavenumbers.
                params (Float[Array, N_PARAMS_AXIS]): Polynomials and Drude
                    parameters.

            Returns:
                Float[Array, "n_wave"]: Combined extinction profile.
            """
            # Extract polynomial coefficients and Drude model parameters from
            # the parameter vector
            poly_coeffs = params[:5]  # First 5 elements for polynomial coeffs
            ...
            drude_result_1 = Drude1d(
                x,
                amplitude=drude_params[0],
                x_0=drude_params[1],
                fwhm=drude_params[2],
            )
            drude_result_2 = Drude1d(
                x,
                amplitude=drude_params[3],
                x_0=drude_params[4],
                fwhm=drude_params[5],
            )
            drude_result_3 = Drude1d(
                x,
                amplitude=drude_params[6],
                x_0=drude_params[7],
                fwhm=drude_params[8],
            )
        ...
        a = jnp.where(
            opt_mask,
            compound_polynomial_drude_model(1 / wave, opt_a),
            a,
        )
        b = jnp.where(
            opt_mask,
            compound_polynomial_drude_model(1 / wave, opt_b),
            b,
        )
        ...
        a = jnp.where(
            optir_overlap,
            (1.0 - weights)
            * compound_polynomial_drude_model(
                1 / wave,
                opt_a,
            )
    ...
    def nirmir_intercept(
        ...
        Args:
            wave (Float[Array, N_WAVE_AXIS]): Wavelengths in micron.
            params (Sequence[float]): Power-law and Drude parameters.

        Returns:
            Float[Array, "n_wave"]: Extinction contribution of the intercept.
        """
...
# TODO: Implement more JAX versions of extinction models from astropy.
# See https://dust-extinction.readthedocs.io/en/latest/index.html
